...
public final class UnsafeExternalBatchSorter extends MemoryConsumer {

  private final Logger logger = LoggerFactory.getLogger(UnsafeExternalBatchSorter.class);
  ...
  @Nullable private volatile UnsafeInMemoryBatchSorter inMemSorter;
  ...
  public static UnsafeExternalBatchSorter create(
      ...
      int initialSize,
      long pageSizeBytes) {
    return new UnsafeExternalBatchSorter(taskMemoryManager, blockManager,
        taskContext, recordComparator, prefixComparator, furtherCompare,
        initialSize, pageSizeBytes, null);
  }

  private UnsafeExternalBatchSorter(
      TaskMemoryManager taskMemoryManager,
      BlockManager blockManager,
      TaskContext taskContext,
      RecordComparator recordComparator,
      ...
      int initialSize,
      long pageSizeBytes,
      @Nullable UnsafeInMemoryBatchSorter existingInMemorySorter) {
    ...
    if (existingInMemorySorter == null) {
      this.inMemSorter = new UnsafeInMemoryBatchSorter(
        this, taskMemoryManager, recordComparator, prefixComparator, needFurtherCompare, initialSize);
    } else {
  ...
  public void startRowBatch() {
    inMemSorter.startRowBatch();
  }

  public void endRowBatch() {
    inMemSorter.endRowBatch();
  }
    ...
    if (spillWriters.isEmpty()) {
      ...
      final UnsafeSorterSpillMerger spillMerger =
        new UnsafeSorterSpillMerger(recordComparator, prefixComparator,
          needFurtherCompare, spillWriters.size());
  ...
  class SpillableIterator extends UnsafeSorterIterator {
    ...
    public SpillableIterator(UnsafeInMemoryBatchSorter.SortedIterator inMemIterator) {
    ...
    public long spill() throws IOException {
      synchronized (this) {
        if (!(upstream instanceof UnsafeInMemoryBatchSorter.SortedIterator && nextUpstream == null
            && numRecords > 0)) {
          return 0L;
        }

        UnsafeInMemoryBatchSorter.SortedIterator inMemIterator =
            ((UnsafeInMemoryBatchSorter.SortedIterator) upstream).clone();
        ...
        long released = 0L;
        synchronized (UnsafeExternalBatchSorter.this) {
